...
import {
  ...
  FolderOpen, Search, User, Bell, Menu, X, ChevronDown, LogOut, Settings,
} from 'lucide-react';
...
export default function WorkerLayout() {
  ...
  useEffect(() => {
    if (user && user.portal && user.portal !== 'worker') {
  ...
  return (
    <div className="min-h-screen bg-[#f0f8f4]">
      {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/worker/dashboard" className="flex items-center gap-3">
              ...
              <div className="hidden sm:block leading-tight">
                <p className="text-sm font-bold tracking-wide text-white">BVI Worker Portal</p>
                <p className="text-[11px] text-green-200">Work Permit &amp; Employment Services</p>
              </div>
            </Link>
          </div>
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.slice(0, 10).map(n => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${!n.read ? 'bg-green-50/50' : ''}`}
                        >
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-200 text-gray-800 overflow-hidden z-50">
                  <div className="px-4 py-3 border-b border-gray-100">
                    ...
                    <span className="inline-block mt-1 text-[10px] uppercase tracking-wider font-bold text-white bg-[#006633] px-2 py-0.5 rounded-full">
                      Worker
                    </span>
                  </div>
                  <div className="py-1">
                    <Link to="/worker/profile" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
        ...
        <aside className={`fixed top-16 left-0 bottom-0 w-64 bg-white border-r border-gray-200 z-40 transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:top-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex flex-col h-full">
            ...
            <div className="px-4 pt-5 pb-3">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-green-100 text-[#006633]">
                <FileCheck size={14} />
                Worker Portal
              </div>
            </div>
              ...
              {navItems.map((item) => (
                <NavLink
                  ...
                  className={isActive(item.to)
                    ? 'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium bg-[#006633] text-white'
            ...
            <div className="p-4 border-t border-gray-100">
              <div className="text-[10px] text-gray-400 leading-relaxed">
                <p className="font-semibold text-gray-500">BVI Worker Portal</p>
